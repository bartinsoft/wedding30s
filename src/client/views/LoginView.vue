  ...
  <div class="min-h-screen bg-gray-950 text-white flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
      <div class="text-center mb-8">
        ...
        <h1 class="font-serif text-2xl">{{ $t('auth.login') }}</h1>
      </div>
      ...
      <div class="mt-6 text-center text-sm text-gray-400 space-y-2">
        <p>
          <router-link to="/forgot-password" class="text-gold-400 hover:text-gold-300 transition-colors">
            {{ $t('auth.forgotPassword') }}
          </router-link>
        </p>
        <p>
          {{ $t('auth.noAccount') }}
          <router-link to="/register" class="text-gold-400 hover:text-gold-300 transition-colors">
            {{ $t('auth.registerLink') }}
          </router-link>
        </p>
      </div>
